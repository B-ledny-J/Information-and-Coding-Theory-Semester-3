...
def generate_matrix_P(k, r):
    ...
    for i in range(1, n + 1):
        # Перевірка, чи число НЕ є степенню двійки (i & (i - 1) != 0)
        if (i & (i - 1)) != 0:
...
def process_hamming_code(m_str, n, k, name):
    """
    Виконує повний цикл кодування, побудови матриць та розрахунку синдрому.
    """
    print("\n" + "=" * 70)
    print(f" ОБЧИСЛЕННЯ ДЛЯ КОДУ ХЕММІНГА {name}")
    print("=" * 70)
    ...
    print(f"1. Вхідне повідомлення m (довжина k={k}):\n   {m}")
    ...
    G = np.hstack((I_k, P))
    print(f"\n2. Породжувальна матриця G ({k}x{n}):\n{G}")

    # 4. Кодування повідомлення: u = m * G (за модулем 2)
    u = np.dot(m, G) % 2
    u_str = "".join(map(str, u))
    print(f"\n3. Закодоване повідомлення u = m * G:\n   {u}  (Рядок: {u_str})")

    # 5. Побудова перевірочної матриці H = [P^T | I_r]
    I_r = np.eye(r, dtype=int)
    H = np.hstack((P.T, I_r))
    print(f"\n4. Перевірочна матриця H ({r}x{n}):\n{H}")

    # 6. Обчислення синдрому помилок S = y * H^T (де y = u)
    y = u
    S = np.dot(y, H.T) % 2
    print(f"\n5. Синдром помилок S = y * H^T:\n   {S}")

    # 7. Висновок
    if np.all(S == 0):
        print("\nВИСНОВОК: Синдром нульовий. Помилок при кодуванні не виявлено.")
    else:
        print("\nВИСНОВОК: Увага! Виявлено помилку в кодовому слові.")


def main():
    print("ПРАКТИЧНА РОБОТА №6: КОДУВАННЯ КОДОМ ХЕММІНГА (ВАРІАНТ 1)")

    process_hamming_code(m_str="1011", n=7, k=4, name="(7,4)")
    process_hamming_code(m_str="10011001110", n=15, k=11, name="(15,11)")
    process_hamming_code(m_str="10011001110111010001000101", n=31, k=26, name="(31,26)")
